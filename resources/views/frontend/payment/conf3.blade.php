    ...
    <section class="form02 content">
        <div class="contentInner">
            ...
            <div class="formWrapper">
                <table class="table01 -resTable">
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <span id="numberData">IS{{ $bill->cris_no ?? '' }}</span>
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                <span id="priceData">¥{{ $bill->invoice_amount ? number_format($bill->invoice_amount) : '' }}</span>
                            </td>
                        </tr>
                        <tr>
                            <th>
                            ...
                            <td>
                                @if (isset($bill->invoice_pdf))
                                    <div class="frameWrapper">
                                        <!-- file=以下にpdfのURLを記述 -->
                                        <iframe width="400" height="300" src="{{asset('_assets/pdf/web/viewer.html?file='). route('show.pdf', ['file_name'=>$bill->invoice_pdf]). '&zoom=page-fit'}}"></iframe>
                                    </div>
                                    <span id="dateData">発行日 {{ $bill->invoice_day ? Carbon::parse($bill->invoice_day)->format('Y年m月d日') : '' }}</span>
                                    <a href="{{ route('show.pdf', ['file_name'=>$bill->invoice_pdf] )}}" target="_blank" rel="noopener noreferrer" class="pdfLink -blank">正しく表示されない場合はこちら</a>
                                @else
                                    データが見つかりません。
                                @endif
                            </td>
                ...
                <div class="sec03" id="submit1">
                    <div class="bunnerContainer">
                        ...
                        <div class="bunnerBox">
                            <h2>ローン支払いで申込む</h2>
                            <!-- //.-disable外すと準備中を外します -->
                            <button class="btnStyle submitBtn" type="button" name="modal" value="confirm" disabled="" id="forModal2"><a href="#modalBox2" data-lity="data-lity">ローン支払いを<br>ご希望の方はこちら</a></button>
                        </div>
                    </div>
                </div>

                <!-- //.-active外すとdisplay:noneします -->
                <div class="sec04 -active" id="submit2">
                ...
                @extends('frontend.layout.paygent')
                <div id="modalBox2" class="lity-hide modalBox">
                    <form action="{{ route('frontend.payment.completed') }}" method="get">
